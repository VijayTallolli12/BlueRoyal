...
- **Strict Rule Hierarchy:**
  ...
  3. **Regular Workday Check (Strict Shift Rule):**
     ...
     - **If NO Shift Assigned (Strict Shift Rule):**
       - Retain raw `actual_hours`.
...
- **Actor:** HR Admin (`attendance:read`) / Super Admin (override).
...
### Workflow 6: Attendance Submission & Approval Workflow (HR Admin Operational Authority)
- **Business Purpose:** Enforce management verification before freezing monthly timekeeping.
- **Actors:**
  - Submitter: `HR Admin` (normal operational authority) or `Super Admin` (override authority) (`attendance:submit`).
  - Approver: `HR Admin` (normal operational authority and final approver) or `Super Admin` (override authority) (`attendance:approve`).
- **Step-by-Step Flow:**
...
- **Actor:** `HR Admin` (normal operational authority) or `Super Admin` (override authority) (`attendance:lock`).
- **Step-by-Step Flow:**
...
- **Actor:** `HR Admin` (normal operational authority) or `Super Admin` (override authority) (`attendance:unlock`).
- **Step-by-Step Flow:**
  1. Required retroactive correction identified on a locked period.
...
- **Business Purpose:** Provide an immutable audit record of every mutation, batch import, and cell correction for corporate governance and regulatory audit.
